...
public class WinScene extends GameScene {
    ...
    public void draw() throws IOException, FontFormatException {
        ...
        if (!model.isOnlineMode()) {
            // offline mode
            view.drawString("player " + String.valueOf(model.getWinnerID()) + " wins!", "lobster", Model.COLORS.BLACK, 34, 100, 500);
        } else {
            if (model.isWinner()) {
                view.drawString("you win!", "lobster", Model.COLORS.BLACK, 34, 100, 500);
            } else {
                view.drawString("opponent wins!", "lobster", Model.COLORS.BLACK, 34, 100, 500);
            }
        }
